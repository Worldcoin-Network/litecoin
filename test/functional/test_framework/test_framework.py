...
TMPDIR_PREFIX = "worldcoin_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    """Base class for a worldcoin test script.

    Individual worldcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a worldcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple worldcoind test nodes"""
        for node in self.nodes:
    ...
    def skip_if_no_bitcoind_zmq(self):
        ...
        if not self.is_zmq_compiled():
            raise SkipTest("worldcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_cli(self):
        ...
        if not self.is_cli_compiled():
            raise SkipTest("worldcoin-cli has not been compiled.")
